...
from google_news_api import GoogleNewsClient, RateLimitError, HTTPError

def test_fetch_feed_retries_on_timeout():
    client = GoogleNewsClient(max_retries=2, retry_backoff=0.01)
    
    with patch.object(client._client, 'get', side_effect=httpx.TimeoutException("timeout")) as mock_get:
